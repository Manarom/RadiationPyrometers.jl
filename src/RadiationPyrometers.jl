...
"""
        Pyrometer(type::String) = begin
            if haskey(pyrometers_types,type) 
                N = length(pyrometers_types[type])
                return new{N}(type,
...
Returns the length of wavelengths vector covered by the [`pyrometers_types`](@ref) (all default pyrometers wavelengh region)
"""
...
function fit_ϵ_wavelength(p::Vector{Pyrometer},Treal::Float64,Tmeasured::Vector{Float64})
        ...
        N = length(p)
        @assert N == length(Tmeasured)
        e_out = Vector{Float64}(undef, length(p))
        Threads.@threads for i = 1:N 
            #(i,(pr,t)) in enumerate(zip(p,Tmeasured))
            e_out[i] = fit_ϵ_wavelength(p[i],Tmeasured[i],Treal)
        end    
        return e_out
    end
    """
